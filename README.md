...
1回の出店を1プロジェクトとして管理するスマホ専用Webアプリです。
イベント・マルシェ・お祭り・文化祭など、食べ物を売る出店ならどれでも扱えます。
...
| 採算シミュレーション | ✅ | 今の価格のまま「何個売ればトントンか」と「目標の手残りに届くには何個か」を計算。想定来場者数から購入率も判定（後述） |
...
- 商品ごとの個数は、**今の「作る予定数」の構成比を保ったまま**全体をスケールして出します（仕込みの段取りを崩さないため）。
  `Q = (固定費 + 目標の手残り) ÷ Σ(1個あたり利益 × 構成比)`、`個数 = ceil(構成比 × Q)`。
- 設定の**目標の手残り**を入れると、トントンの先＝**その額を残すには全部で何個売るか**を同じ式で逆算し、
  「トントンから あと何個」を添えて表示します。**損益分岐点は「目標の手残り0円」の特殊ケース**として
  同じ関数を通しており、「赤字を防ぐ」と「利益を読む」で計算を二重に持ちません。
  未入力なら逆算そのものを出しません（想定来場者数が未入力なら購入率を出さないのと同じ方針）。
  かかるお金が0でも、目標があれば逆算できます。
...
- **ひとつ前に戻せる**：上書きは取り消せないので、「これにする」の**直前の状態を控えとして自動保存**します
  （`source: "auto"`。控えの保存と上書きは**同じ batch** ＝ 撮り損ねたまま上書きされることがない）。
  控えは**1件だけ**持ち押すたびに上書きするので、**戻せるのは1手前まで**。
  手動パターンの5件の枠には数えません（勝手に増えた1件で枠が埋まらないように）。
  控えを適用すると、そのとき控えに入るのは「戻す直前＝適用後の状態」になるので、もう一度押せば行き来できます。
  控えのカードは点線・グレーで、編集とスワイプ削除は出さず、ボタンだけ「ひとつ前に戻す」に変わります。
  なお**「今の値段をパターンとして先に保存しておく」方式は採りませんでした**。
  保存した瞬間の値で固まるため、あとでレシピの値段を直接変えると
  「元の値段」という名前のまま中身がズレていくためです（比較表の「今」列は毎回読み直すのでこの問題がない）。
