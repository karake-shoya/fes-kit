    ...
    <div className="flex flex-col gap-6">
      ...
      {tab === "calendar" ? (
        ...
        <div className="flex flex-col gap-4">
          {/* グループ方法の切り替え */}
          <div className="flex items-center gap-1 self-end rounded-lg bg-zinc-100 p-0.5 text-xs">
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center gap-1.5 rounded-lg py-2 text-sm font-medium transition-colors ${
        active ? "bg-white text-amber-800 shadow-sm" : "text-zinc-500"
      }`}
    >
      {children}
    </button>
      ...
      {groups.map(([status, items]) => (
        <section key={status} className="flex flex-col gap-2">
          <h2 className={`text-sm font-semibold px-1 ${STATUS_STYLE[status].text}`}>
            ...
            <span className="ml-1 text-xs text-zinc-400">{items.length}</span>
          </h2>
          <ul className="flex flex-col gap-2">
            {items.map((s) => (
              <ScheduleCard key={s.id} schedule={s} projectId={projectId} canEdit={canEdit} />
